...
Verified on 2026-06-09: the Apps Script project editor opens, but the visible `Code.gs` is still the older script. It has `doPost` and the first v2 run columns, but it does not include `doGet`, `SCHEMA_VERSION = "temperature-daq-v2"`, or the per-channel diagnostic headers.

That matches the live endpoint behavior: `GET ?mode=health` returns an HTML Apps Script error saying `Script function not found: doGet`.

Next action: replace the editor contents with the repo copy at `apps_script/Code.gs`, save, then update or create the web app deployment. After that, rerun:
