...
int nodeID = 1; // Unique ID for this sensor node (1-8)
...
void loop() {
  // basic readout test, just print the current temp
   ...
   if(millis() - lastSendTime >= sendInterval) {
    ...
    Serial.println("Sendinf from Temp " + String(nodeID) + ": " + String(temp) + " C");
    ...
    float temp = c;
    String jsonData = "{\"method\":\"append\",\"temp1\":" + String(temp) + "}";
